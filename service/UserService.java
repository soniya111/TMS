...
public class UserService implements UserServiceIntr {
	...
	public APIResponse updateHist(LearningHistory l, String histId) {
		...
		if (present == true) {
			LearningHistory afterReg = histid.get();

			LocalDateTime now = LocalDateTime.now();
			afterReg.setGrade("pass");
			afterReg.setCompletionDateTime(now);
			afterReg.setCompletionStatus(l.getCompletionStatus()); //

			learningHistoryRepo.save(afterReg);
	...
	public APIResponse updateCourses(CourseDetails c, String courseid) {
		APIResponse apiResponse = new APIResponse();

		Optional<CourseDetails> cid = courseDetailsRepo.findById(courseid);
		boolean present = cid.isPresent();
		if (present == true) {
			CourseDetails course = cid.get();
			course.setCourseName(c.getCourseName());
			course.setCourseDescr(c.getCourseDescr());
			course.setCourseTitle(c.getCourseTitle());
			course.setContentType(c.getContentType());
			course.setVersion(c.getVersion());

			courseDetailsRepo.save(course);
			apiResponse.setData("course updated successfully");
			return apiResponse;
		} else {
			apiResponse.setData("courseId is wrong");
		}
		return apiResponse;
	}
